...
impl FileTreeSidebar {
    ...
    pub(crate) fn refresh_git_statuses(&mut self, diff_providers: &DiffProviderRegistry) {
        ...
        diff_providers.clone().for_each_changed_file(root, move |result| {
    ...
    pub(crate) fn click_at_row(&mut self, area: Rect, row: u16) -> FileTreeEvent {
        ...
        let is_double_click = self
            .last_click
            .as_ref()
            .is_some_and(|(path, instant)| path == &entry.path && now.duration_since(*instant) <= DOUBLE_CLICK_THRESHOLD);
    ...
    pub(crate) fn render(
        ...
        let diagnostics = diagnostic_counts_by_path(editor);
        let active_path = current_ref!(editor).1.path().map(normalize);
        let statuses = lock(&self.git_statuses).clone();
            ...
            .enumerate()
        {
            ...
            let counts = diagnostics.get(&entry.path).copied().unwrap_or_default();
    ...
    fn rebuild(&mut self) {
        ...
        for (child_path, child_kind) in self.read_children(&self.root) {
            match child_kind {
                ...
                EntryKind::File => self.entries.push(FileTreeEntry {
                    path: child_path,
                    depth: 0,
                    kind: EntryKind::File,
                    expanded: false,
    ...
    fn read_children(&self, dir: &Path) -> Vec<(PathBuf, EntryKind)> {
        let absolute_root = self.root.canonicalize().unwrap_or_else(|_| self.root.clone());
        ...
        content.sort_by(|(left_path, left_kind), (right_path, right_kind)| {
            ...
            left_rank.cmp(&right_rank).then_with(|| left_path.cmp(right_path))
        });
    ...
    fn entry_label(&self, entry: &FileTreeEntry) -> String {
        ...
        let icon = match entry.kind {
            EntryKind::Directory if entry.expanded => "󰷏",
            EntryKind::Directory => "󰉋",
            EntryKind::File => file_icon(&entry.path),
        };
...
mod tests {
    ...
    fn initializes_with_first_level_entries_only() {
        ...
        assert!(sidebar.entries.iter().any(|entry| entry.path.ends_with("src")));
        assert!(sidebar
            .entries
            .iter()
    ...
    fn double_click_opens_file() {
        ...
        assert!(matches!(sidebar.click_at_row(area, row), super::FileTreeEvent::Consumed));
        assert!(matches!(sidebar.click_at_row(area, row), super::FileTreeEvent::Open(path) if path == file));
    }
